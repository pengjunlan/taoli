...
class AccountPresenter(AccountServiceSupport):
    """Formats account-domain query data into the existing page/API payload shapes."""
    ...
    def build_account_rows(
        ...
        *,
        guard_states: Dict[int, Dict[str, object]] | None = None,
    ) -> List[Dict[str, str]]:
        result: List[Dict[str, str]] = []
        guard_states = guard_states or {}
            ...
            result.append(
                {
                    ...
                    "fee_rate_synced_at": self._format_datetime(row.get("fee_rate_synced_at")) if row.get("fee_rate_synced_at") else "--",
    ...
    def build_address_rows(self, rows: List[Dict[str, object]]) -> List[Dict[str, str]]:
        result: List[Dict[str, str]] = []
        for row in rows:
    ...
    def build_balance_rows(
        ...
        for row in account_rows:
            ...
            available_precise = self._format_currency_with_precision(balance_snapshot.available_value, precision=4)
            available_value = int(round(balance_snapshot.available_value))
            funding_ratio_percent = float(row.get("funding_ratio_percent") or 0)
            available_rows.append((row, available_value, available, available_precise, funding_ratio_percent))
            total_available_value += available_value

        total_target_pool_value = total_available_value
        result: List[Dict[str, str]] = []

        for row, available_value, available, available_precise, funding_ratio_percent in available_rows:
            exchange = str(row.get("exchange") or "")
            current_total_amount_value = float(row.get("current_total_amount") or 0)
            current_balance_precise = self._format_currency_with_precision(current_total_amount_value, precision=4)
            ...
            result.append(
                {
                    ...
                    "available": available,
                    "available_precise": available_precise,
                    "current_balance": self._format_amount(int(round(current_total_amount_value))),
                    "current_balance_precise": current_balance_precise,
    ...
    def build_transfer_rows(self, rows: List[Dict[str, object]]) -> List[Dict[str, str]]:
        result: List[Dict[str, str]] = []
        for row in rows:
            ...
            result.append(
                {
                    ...
                    "to_network": self._network_label(str(row.get("to_network") or "")) or "未配置",
                    "to_address": str(row.get("to_address_value") or "").strip() or "--",
    ...
    def build_transfer_options(
        ...
        *,
        ...
        for row in rows:
            if int(row["id"]) == int(from_account["id"]):
                continue
        ...
        supported_count = int(transfer_source_summary["transfer_option_count"] or 0)
        notice = (
            MANUAL_TRANSFER_UI_NOTICE
    ...
    def _format_currency_with_precision(self, value: float, precision: int = 4) -> str:
        text = f"{float(value or 0):,.{precision}f}".rstrip("0").rstrip(".")
