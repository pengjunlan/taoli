...
from app.views.viewmodels.pages.page_contexts import spread_rows


router = APIRouter()

PAGE = PageConfig(
    key="spread_arbitrage",
    template_name="pages/spread_arbitrage.html",
    title="跨交易所价差套利",
    subtitle="自动扫描已配置交易所中的价差机会，命中统一规则后直接进入套利执行。",
    css_name="spread_arbitrage.css",
    js_name="spread_arbitrage.js",
)
...
async def spread_arbitrage_page(request: Request) -> HTMLResponse:
    ...
    return render_page(request, PAGE, current_user=current_user, rows=spread_rows())
